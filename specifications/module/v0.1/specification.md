...
A set of random, game-instance–specific values chosen by the hub static at the beginning of each game or dynamic while the game is running. Environment parameters are physically presented to the player and may be observed and interpreted as part of solving module riddles.
...
A module MUST comply with the following specifications for all environment parameters defined in the environment_parameters specification. A module MUST comply with the hub_module_connection specification on how modules are informed about and can ask for values of environment_parameters.   

A module MAY also provide its own environment parameters for the same purpose as the environment parameters provided by the hub. These internal environment parameters MUST NOT be exposed to other devices. 
